...
Начиная с `olcrtc-2.0.27` клиент поддерживает QR-бандлы подписок из Admin UI сервера `server-v1.9.35+`. Такой QR содержит:
...
При сканировании QR-бандла приложение создаёт группу типа **Subscription**, сохраняет URL подписки и сразу добавляет все профили из snapshot. Это нужно для bootstrap-сценария, когда сервер подписок не доступен с мобильной сети до поднятия olcRTC-туннеля.
...
Starting with `olcrtc-2.0.27`, the client supports subscription QR bundles generated by Admin UI in `server-v1.9.35+`. A bundle contains:
...
When such a QR is scanned, the app creates a **Subscription** group, stores the subscription URL and immediately imports every profile from the snapshot. This is the bootstrap path for networks where the subscription server is not reachable before the olcRTC tunnel is up.
