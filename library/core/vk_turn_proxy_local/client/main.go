...
func Start(ctx context.Context, opts Options) error {
	if opts.Listen == "" {
		opts.Listen = "127.0.0.1:9000"
	}
	if opts.DNSMode == "" {
		opts.DNSMode = dnsdial.DNSModeAuto
	}
	if opts.DNSMode != dnsdial.DNSModeUDP && opts.DNSMode != dnsdial.DNSModeDoH && opts.DNSMode != dnsdial.DNSModeAuto {
		return fmt.Errorf("invalid dns mode %q: must be udp | doh | auto", opts.DNSMode)
	}
	if opts.DNSServers != "" {
		servers := strings.Split(opts.DNSServers, ",")
		if len(servers) > 0 {
			dnsdial.SetUDPDNSServers(servers)
		}
	}
	appDialer = dnsdial.AppDialer(opts.DNSMode)
	dnsdial.InstallGlobalResolver(opts.DNSMode)
	if opts.PeerAddr == "" {
	...
	if opts.Wrap && opts.NoDTLS {
		return fmt.Errorf("-wrap requires DTLS")
	}
	var err error
	var wrapKey []byte
	if opts.Wrap {
		wrapKey, err = decodeWrapKey(opts.Wrap, opts.WrapKeyHex)
		if err != nil {
			return err
		}
	}
	if opts.StreamsPerCred > 0 {
		streamsPerCache = opts.StreamsPerCred
	} else if opts.StreamsPerCred < 0 {
		return fmt.Errorf("streams per credential must be positive")
	}
	...
	linkParts := strings.Split(opts.VKLink, "join/")
	...
	if opts.VLESS {
		runVLESSMode(ctx, params, peer, opts.Listen, n, opts.VLESSBond, opts.Ready)
		return ctx.Err()
	}
...
func RunCLI() {
	...
	if *vlessMode {
		runVLESSMode(ctx, params, peer, *listen, *n, *vlessBond, nil)
		return
	}
...
func runVLESSMode(ctx context.Context, tp *turnParams, peer *net.UDPAddr, listenAddr string, numSessions int, bond bool, ready chan<- struct{}) {
	pool := &sessionPool{}
