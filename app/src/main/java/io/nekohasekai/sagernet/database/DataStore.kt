...
object DataStore : OnPreferenceDataStoreChangeListener {
    ...
    var serverOlcrtcRoomPassword by profileCacheStore.string(Key.SERVER_OLCRTC_ROOM_PASSWORD)
    ...
    /**
     * Per-profile olcRTC `clientID`. Sourced from the URI/QR (`client_id=`)
     * generated by the server admin panel (see requirements-server.md S8).
     * Read-only in the UI: the client SHALL NOT generate this value locally
     * because the server uses `fnv32(clientID)` as the vp8channel binding
